...
public class MyController extends AbstractVerticle {
    ...
    private void factorial(RoutingContext routingContext) {
        HttpServerResponse response = routingContext.response();
        HttpServerRequest request = routingContext.request();
        String mode = request.getParam("fac");
	BigInteger modeFac = new BigInteger(mode);
	BigInteger res = factorial(modeFac);
        String jsonResponse = procesaFac(res.toString(), request);
        response.setStatusCode(200).
        putHeader("content-type", "application/json; charset=utf-8").
        end(jsonResponse);
    	}
    private BigInteger factorial (BigInteger numero) {
	BigInteger uno = new BigInteger("1");
	BigInteger cero = new BigInteger("0");
	int res = numero.compareTo(cero);
	if (res == 0) 
		return uno;
	else 
    ...
    private String procesaFac(String res, HttpServerRequest req) {
        Map<Object, Object> info = new HashMap<>();
        info.put("servidor", req.localAddress().host());
        info.put("operacion", "FactorialNumeros");
        info.put("numeros", res.length());
        info.put("resultado", res);
        return Json.encodePrettily(info);
    }
